...
if [ -n "${bundle+x}" ];              then didbuild=1; if [ "$host_os" != "Darwin" ]; then echo "[ERROR] bundle target is only supported on Darwin."; exit 1; fi; if [ -z "${raddbg+x}" ]; then $compile ../src/raddbg/raddbg_main.c $compile_link $link_os_gfx $link_render $link_font_provider $out raddbg; sign_raddbg_debug raddbg; fi; rm -rf "RAD Debugger.app"; mkdir -p "RAD Debugger.app/Contents/MacOS" "RAD Debugger.app/Contents/Resources"; cp ../src/mac/raddbg_Info.plist "RAD Debugger.app/Contents/Info.plist"; cp ../src/mac/raddbg.icns "RAD Debugger.app/Contents/Resources/raddbg.icns"; cp raddbg "RAD Debugger.app/Contents/MacOS/raddbg"; chmod +x "RAD Debugger.app/Contents/MacOS/raddbg"; sign_raddbg_debug "RAD Debugger.app/Contents/MacOS/raddbg"; sign_raddbg_debug "RAD Debugger.app"; fi
if [ -n "${radbin+x}" ];              then didbuild=1 && $compile ../src/radbin/radbin_main.c                                    $compile_link $out radbin; fi
if [ -n "${torture+x}" ];             then didbuild=1 && $compile ../src/torture/torture_main.c                                  $compile_link $link_os_gfx $link_render $link_font_provider $out torture; fi
