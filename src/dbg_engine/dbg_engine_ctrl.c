...
d_macho_unwind_info_data_from_module(Arena *arena, D_Handle module_handle)
{
  ...
  MutexScopeR(stripe->rw_mutex) for(D_ModuleImageInfoCacheNode *n = slot->first; n != 0; n = n->next)
  {
    if(d_handle_match(n->module, module_handle))
    {
      result = push_str8_copy(arena, n->macho_unwind_info_data);
      break;
    }
  }
  return result;
...
typedef struct D_MemoryReadContextDwarf D_MemoryReadContextDwarf;
struct D_MemoryReadContextDwarf
{
...
{
  D_MemoryReadContextDwarf *ctx = ud;
  B32 is_stale = 0;
    ...
    if(is_cfi_parsed && contains_1u64(fde.pc_range, ip))
    {
      ...
      if(cfi_row)
      {
        ...
        switch(arch)
        {
          ...
          case Arch_arm64:
          {
            D_MemoryReadContextDwarf *mem_read_ctx_dwarf = push_array(scratch.arena, D_MemoryReadContextDwarf, 1);
            mem_read_ctx_dwarf->process_handle = process_handle;
            mem_read_ctx_dwarf->endt_us        = endt_us;
            mem_read_ctx = mem_read_ctx_dwarf;
...
d_unwind_from_thread(Arena *arena, D_Handle thread, U64 endt_us)
{
  ...
  if(regs_block_good)
  {
    for(unwind.flags = 0; frame_node_count < 256;)
    {
      ARCH_Info *arch_info = arch_info_from_arch(arch);
      ...
      D_UnwindStepResult frame_ctx_result = {0};
      switch(process_entity->target_os)
      {
        ...
        case OperatingSystem_Mac:
        {
          ...
          if(arch == Arch_x64 && module_entity != 0 && !(frame_ctx_result.flags & D_UnwindFlag_Stale))
          {
          ...
          else if(arch == Arch_arm64 && module_entity != 0 && !(frame_ctx_result.flags & D_UnwindFlag_Stale))
          {
            ...
            else if(compact_dwarf)
            {
              ...
              if(eh_frame_vaddr != 0)
              {
                U64 fde_addr = eh_frame_vaddr + (lookup.encoding & MACHO_UNWIND_ARM64_DWARF_SECTION_OFFSET);
                D_FrameUnwindContext compact_dwarf_frame_ctx = {0};
                frame_ctx_result = d_establish_frame_unwind_context__dwarf_from_fde_addr(arena, process_entity->handle, module_entity->handle, arch, regs_block, fde_addr, endt_us, &compact_dwarf_frame_ctx);
                if(!(frame_ctx_result.flags & (D_UnwindFlag_Error|D_UnwindFlag_Stale)))
                {
      ...
      D_UnwindFrame *frame = &frame_node->v;
      ...
      switch(process_entity->target_os)
      {
        ...
        case OperatingSystem_Mac:
        {
          switch(arch)
          {
            ...
            case Arch_x64:
            {
              step_result = d_unwind_step__macho_x64(process_entity->handle, module_entity->handle, module_entity->vaddr_range.min, regs_block, endt_us);
            }break;
            case Arch_arm64:
            {
              ARM64_RegBlock *regs_arm64 = regs_block;
...
d_ctrl_thread__append_resolved_module_user_bp_traps(Arena *arena, D_EvalScope *eval_scope, D_Handle process, D_Handle module, D_BreakpointList *user_bps, DMN_TrapChunkList *traps_out)
{
  ...
  for(D_BreakpointNode *n = user_bps->first; n != 0; n = n->next)
  {
    ...
    if(bp->file_path.size != 0)
    {
      ...
      if(traps_out->trap_count == trap_count_before)
      {
        U64 src_file_count = 0;
        RDI_SourceFile *src_files = rdi_table_from_name(rdi, SourceFiles, &src_file_count);
        for(U64 src_idx = 1; src_idx < src_file_count; src_idx += 1)
        {
          RDI_SourceFile *src = &src_files[src_idx];
          U64 src_path_size = 0;
          U8 *src_path_str = rdi_normal_path_from_source_file(rdi, src, &src_path_size);
          String8 src_path = str8(src_path_str, src_path_size);
          if(src_path.size == 0 || !str8_ends_with(filename_normalized, src_path, 0))
          {
            continue;
          }
        ...
        if(hit_trap_flags & D_TrapFlag_SingleStepAfterHit) LogInfoNamedBlockF("trap_net__single_step_after_hit")
        {
          ...
          if(!single_step_stop)
          {
            ...
            if(post_step_trap_flags != 0)
            {
              ...
              if(!(pre_single_step_trap_flags & D_TrapFlag_SaveStackPointer))
              {
                stack_pointer_matches = (d_sp_from_thread(target_thread) == sp_check_value);
              }
            }
          }
        }
