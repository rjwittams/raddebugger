...
internal void
d_ctrl_thread__module_open(D_Handle process, D_Handle module, Rng1U64 vaddr_range, String8 path, DMN_ModuleInfo *module_info)
{
  Temp scratch = scratch_begin(0,0);
  U64 base_vaddr = vaddr_range.min;
  ...
  if(module_info == 0 || module_info == &dmn_module_info_nil)
  {
    ...
    module_info->module_path = str8_copy(arena, path);
    module_info->debug_info_path = str8_copy(arena, path);
    module_info->vsize = dim_1u64(vaddr_range);
    U32 magic = 0;
    if(d_process_read_struct(process, base_vaddr, &magic) == sizeof(magic) &&
       macho_magic_is_supported(magic))
    {
      U64 header_size = macho_header_size_from_magic(magic);
      MachO_Header64 header = {0};
      if(header_size != 0 &&
         d_process_read(process, r1u64(base_vaddr, base_vaddr + header_size), &header) == header_size &&
         header.load_commands_size < MB(16))
      {
        ...
        if(d_process_read(process, r1u64(base_vaddr, base_vaddr + data_size), data_bytes) == data_size)
        {
...
d_ctrl_thread__next_dmn_event(Arena *arena, DMN_CtrlCtx *ctrl_ctx, D_Msg *msg, DMN_RunCtrls *run_ctrls, D_Spoof *spoof)
{
  ...
  ProfScope("push ctrl events associated with this demon event") switch(event->kind)
  {
    ...
    case DMN_EventKind_LoadModule:
    {
      ...
      d_ctrl_thread__module_open(process_handle, module_handle, r1u64(event->address, event->address+event->size), module_path, event->module_info);
...
d_ctrl_thread__open_crash_dump(DMN_CtrlCtx *ctrl_ctx, D_Msg *msg)
{
  ...
  for EachIndex(idx, dump_modules_count)
  {
    ...
    d_ctrl_thread__module_open(process, module_handle, vaddr_range, dump_modules[idx].path, 0);
#endif
    // rjf: open debug info
    String8 initial_debug_info_path = d_initial_debug_info_path_from_module(scratch.arena, module_handle);
    U64 debug_info_timestamp = properties_from_file_path(initial_debug_info_path).modified;
