...
internal DMN_TrapFlags d_dmn_trap_flags_from_breakpoint_flags(D_BreakpointFlags flags);
internal D_BreakpointFlags d_breakpoint_flags_from_dmn_trap_flags(DMN_TrapFlags flags);
...
internal void d_trap_list_push(Arena *arena, D_TrapList *list, D_Trap *trap);
internal D_TrapList d_trap_list_copy(Arena *arena, D_TrapList *src);

////////////////////////////////
//~ rjf: User Breakpoint Type Functions

internal void d_breakpoint_list_push(Arena *arena, D_BreakpointList *list, D_Breakpoint *bp);
internal D_BreakpointList d_breakpoint_list_copy(Arena *arena, D_BreakpointList *src);

////////////////////////////////
//~ rjf: Message Type Functions

//- rjf: deep copying
internal void d_msg_deep_copy(Arena *arena, D_Msg *dst, D_Msg *src);

//- rjf: list building
internal D_Msg *d_msg_list_push(Arena *arena, D_MsgList *list);
internal D_MsgList d_msg_list_deep_copy(Arena *arena, D_MsgList *src);
internal void d_msg_list_concat_in_place(D_MsgList *dst, D_MsgList *src);

//- rjf: serialization
internal String8 d_serialized_string_from_msg_list(Arena *arena, D_MsgList *msgs);
internal D_MsgList d_msg_list_from_serialized_string(Arena *arena, String8 string);

////////////////////////////////
//~ rjf: Event Type Functions

//- rjf: list building
internal D_Event *d_event_list_push(Arena *arena, D_EventList *list);
internal void d_event_list_concat_in_place(D_EventList *dst, D_EventList *to_push);

//- rjf: serialization
internal String8 d_serialized_string_from_event(Arena *arena, D_Event *event, U64 max);
internal D_Event d_event_from_serialized_string(Arena *arena, String8 string);
...
internal Rng1U64 d_voff_range_from_vaddr_range(D_Entity *module, Rng1U64 vaddr_range);
internal B32 d_entity_tree_is_frozen(D_Entity *root);
...
internal D_EntityCtxRWStore *d_entity_ctx_rw_store_alloc(void);
internal void d_entity_ctx_rw_store_release(D_EntityCtxRWStore *store);

//- rjf: string allocation/deletion
internal U64 d_name_bucket_num_from_string_size(U64 size);
internal String8 d_entity_string_alloc(D_EntityCtxRWStore *store, String8 string);
internal void d_entity_string_release(D_EntityCtxRWStore *store, String8 string);

//- rjf: entity construction/deletion
internal D_Entity *d_entity_alloc(D_EntityCtxRWStore *store, D_Entity *parent, D_EntityKind kind, Arch arch, D_Handle handle, U64 id);
internal void d_entity_release(D_EntityCtxRWStore *store, D_Entity *entity);

//- rjf: entity equipment
internal void d_entity_equip_string(D_EntityCtxRWStore *store, D_Entity *entity, String8 string);
...
internal void d_set_wakeup_hook(D_WakeupFunctionType *wakeup_hook);
...
typedef struct D_MachOCompactArm64FrameRegSlot D_MachOCompactArm64FrameRegSlot;
struct D_MachOCompactArm64FrameRegSlot
{
  ARM64_RegCode reg_code;
  S32 cfa_off;
  U32 byte_size;
};
...
internal D_UnwindStepResult d_unwind_step__frame_pointer_arm64(D_Handle process_handle, ARM64_RegBlock *regs, U64 endt_us);
internal D_UnwindStepResult d_unwind_step__macho_arm64(D_Handle process_handle, D_Handle module_handle, U64 module_base_vaddr, ARM64_RegBlock *regs, B32 prefer_entry_lr, U64 endt_us);
...
internal D_CallStack d_call_stack_from_unwind(Arena *arena, D_Entity *process, D_Unwind *base_unwind);
internal D_CallStackFrame *d_call_stack_frame_from_unwind_and_inline_depth(D_CallStack *call_stack, U64 unwind_count, U64 inline_depth);

////////////////////////////////
//~ rjf: Halting All Attached Processes

internal void d_halt(void);

////////////////////////////////
//~ rjf: Shared Accessor Functions

//- rjf: generation counters
internal U64 d_run_gen(void);
internal U64 d_mem_gen(void);
internal U64 d_reg_gen(void);
...
internal B32 d_u2c_push_msgs(D_MsgList *msgs, U64 endt_us);
internal D_MsgList d_u2c_pop_msgs(Arena *arena);

//- rjf: control -> user thread communication
internal void d_c2u_push_events(D_EventList *events);
internal D_EventList d_c2u_pop_events(Arena *arena);

//- rjf: entry point
internal void d_ctrl_thread__entry_point(void *p);
...
internal void d_ctrl_thread__module_open(D_Handle process, D_Handle module, Rng1U64 vaddr_range, String8 path, DMN_ModuleInfo *module_info);
...
internal void d_ctrl_thread__eval_scope_end(D_EvalScope *scope);

//- rjf: log flusher
internal void d_ctrl_thread__end_and_flush_log(void);
