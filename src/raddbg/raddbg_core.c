        ...
        switch(kind)
        {
          ...
          case RD_CmdKind_RunExternalDriverTextCommand:
          {
            ...
            if(str8_match(cmd_name, str8_lit("darwin_tlv_probe"), 0))
            {
              ...
              D_Entity *thread = rd_ctrl_entity_from_string(thread_or_process_string, D_EntityKind_Thread);
              if(thread == &d_entity_nil)
              {
...
#undef Z
          ...
          //- debugger diagnostic IPC commands
          case RD_CmdKind_DiagnoseDemonState:
          {
            rd_cmd_output_clear();
            String8List lines = {0};
#if OS_MAC
            B32 access_opened = dmn_access_open();
            str8_list_pushf(scratch.arena, &lines, "demon:mac access:%u ctrl_running:%u access_run_state:%u halt_requested:%u",
            ...
            if(access_opened && mac_dmn_state != 0)
            {
              U64 process_count = 0;
              U64 detach_count = 0;
              ...
              for(MAC_DMN_Entity *process_entity = mac_dmn_state->first_process_entity; process_entity != 0; process_entity = process_entity->next)
              {
                ...
                U64 thread_count = 0;
                U64 module_count = 0;
                for(MAC_DMN_Entity *child = process->first_thread_entity; child != 0; child = child->next)
                {
                  if(child->kind == MAC_DMN_EntityKind_Thread)
                  {
                    thread_count += 1;
                  }
                }
                for(MAC_DMN_Entity *child = process->first_module_entity; child != 0; child = child->next)
                {
                  if(child->kind == MAC_DMN_EntityKind_Module)
                  {
                    module_count += 1;
                  }
                }
                str8_list_pushf(scratch.arena, &lines,
                                "\nprocess handle:%S pid:%i task:0x%x arch:%S attached:%u running:%u halt_expected:%u needs_attach_events:%u mach_exceptions:%u pending_exception:{valid:%u exception:%i code0:%I64u code1:%I64u thread_port:0x%x} threads:%I64u modules:%I64u",
                                ...
                                process->pending_exception.thread, thread_count, module_count);
                U64 thread_idx = 0;
                for(MAC_DMN_Entity *thread_entity = process->first_thread_entity; thread_entity != 0; thread_entity = thread_entity->next)
                {
                  if(thread_entity->kind != MAC_DMN_EntityKind_Thread) { continue; }
                  ...
                  str8_list_pushf(scratch.arena, &lines,
                                  ...
                                  thread->thread_id, thread->thread, thread->thread != MACH_PORT_NULL,
                                  thread->is_suspended_for_run, thread->is_stepping_over_dyld_notification, thread->is_stepping_over_debug_trap,
                                  thread->dyld_notification_step_vaddr, basic_result, basic.run_state,
            ...
            {
              str8_list_pushf(scratch.arena, &lines, "\nbackend snapshot unavailable while daemon has exclusive run access; halt first for exact demon state");
            }
#else
            str8_list_pushf(scratch.arena, &lines, "demon diagnostics unavailable on this platform");
#endif
