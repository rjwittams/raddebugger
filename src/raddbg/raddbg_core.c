          ...
          case RD_CmdKind_RunExternalDriverTextCommand:
          {
            ...
            else if(str8_match(cmd_name, str8_lit("dump_output"), 0))
            {
              ...
              if(msg_parts.first != 0 && msg_parts.first->next != 0)
              {
                ...
                if(try_u64_from_str8_c_rules(msg_parts.first->next->string, &parsed_max_bytes))
                {
            ...
            else if(str8_match(cmd_name, str8_lit("select_process"), 0))
            {
              ...
              {
                rd_cmd(RD_CmdKind_SelectEntity, .ctrl_entity = process->handle);
            ...
            else if(str8_match(cmd_name, str8_lit("dump_call_stack"), 0))
            {
              ...
              D_Entity *thread = &d_entity_nil;
              if(thread_or_process_string.size != 0)
              {
                ...
                {
                  U64 pid = 0;
                  if(try_u64_from_str8_c_rules(thread_or_process_string, &pid))
                  {
                    D_EntityArray processes = d_entity_array_from_kind(D_EntityKind_Process);
                    for(U64 idx = 0; idx < processes.count; idx += 1)
                    {
