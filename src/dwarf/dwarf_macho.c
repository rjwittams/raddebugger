...
{
  DW_SectionKind result = DW_SectionKind_Null;
  for(DW_SectionKind section_kind = DW_SectionKind_Null+1; section_kind < DW_SectionKind_COUNT; section_kind += 1)
  {
    String8 mach_name = dw_macho_name_from_section_kind(section_kind);
  ...
  if(bin->is_64)
  {
    for EachIndex(command_idx, bin->load_commands.count)
    {
      ...
      for(U64 section_idx = 0; section_idx < segment.section_count && section_offset + sizeof(MachO_Section64) <= info->offset + info->cmd_size; section_idx += 1, section_offset += sizeof(MachO_Section64))
      {
        ...
        if(dw_section_kind_from_macho_name(section_name) != DW_SectionKind_Null)
        {
          result = 1;
          goto done;
        }
...
internal DW_Raw
dw_raw_from_macho_bin(Arena *arena, String8 data, MachO_Bin *bin)
{
  DW_Raw result = {0};
  ...
  if(bin->is_64)
  {
    for EachIndex(command_idx, bin->load_commands.count)
    {
      ...
      for(U64 section_idx = 0; section_idx < segment.section_count && section_offset + sizeof(MachO_Section64) <= info->offset + info->cmd_size; section_idx += 1, section_offset += sizeof(MachO_Section64))
      {
        ...
        if(section_kind == DW_SectionKind_Null || is_section_present[section_kind])
        {
          continue;
        }
