...
barrier_wait(Barrier barrier)
{
  ...
  if(entity != 0)
  {
    ...
    entity->barrier.crossing_count += 1;
    if(entity->barrier.crossing_count >= entity->barrier.count)
    {
      entity->barrier.crossing_count = 0;
...
copy_file_path(String8 dst, String8 src)
{
     ...
     !file_match(dst_h, file_zero()))
  {
    ...
    {
      U64 read_size = Min(sizeof(buffer), bytes_left_to_copy);
      ssize_t read_result = read(src_fd, buffer, read_size);
      if(read_result <= 0)
      {
        break;
      }
      ssize_t write_result = write(dst_fd, buffer, (U64)read_result);
      if(write_result != read_result)
      {
        break;
      }
      bytes_left_to_copy -= (U64)read_result;
    }
    result = (bytes_left_to_copy == 0);
  }
  file_close(src_h);
  file_close(dst_h);
  return result;
    ...
    //- grab dynamically allocated process info
    {
      ...
      // grab binary path
      {
        ...
        for(S64 cap = PATH_MAX, r = 0; r < 4; cap *= 2, r += 1)
        {
          ...
          U32 size32 = (U32)cap;
          int result = _NSGetExecutablePath((char *)buffer, &size32);
          size = cstring8_length(buffer);
          if(result == 0 && size < cap)
          {
            got_final_result = 1;
            break;
          }
